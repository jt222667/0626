                   ...
                   real_T C[7], real_T G[7])
{
  ...
  if (loop_ub < 3200) {
    for (i = 0; i < loop_ub; i++) {
      ...
      if (((int8_T)W_d11_idx_0_tmp < 1) || ((int8_T)W_d11_idx_0_tmp > 7)) {
        emlrtDynamicBoundsCheckR2012b((int8_T)W_d11_idx_0_tmp, 1, 7, &emlrtBCI,
                                      (emlrtConstCTX)sp);
      }
...
#pragma omp for nowait
      for (i = 0; i < loop_ub; i++) {
        ...
        if (setjmp(emlrtJBEnviron) == 0) {
          ...
          if (((int8_T)d < 1) || ((int8_T)d > 7)) {
            emlrtDynamicBoundsCheckR2012b((int8_T)d, 1, 7, &emlrtBCI, &st);
          }
          tmp_data[i] = (int8_T)d;
        } else {
          emlrtHadParallelError = true;
  ...
  /*  质量矩阵强制对称化 */
  for (b_i = 0; b_i < 7; b_i++) {
    for (dyn_case = 0; dyn_case < 7; dyn_case++) {
      R1_tmp = dyn_case + 7 * b_i;
      dv[R1_tmp] = 0.5 * (M[R1_tmp] + M[b_i + 7 * dyn_case]);
    }
  }
  memcpy(&M[0], &dv[0], 49U * sizeof(real_T));
}
